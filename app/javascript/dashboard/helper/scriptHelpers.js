...
export const initializeWoodeskEvents = () => {
  emitter.on(WOODESK_RESET, () => {
    if (window.$woodesk) {
      window.$woodesk.reset();
    }
  });
  emitter.on(WOODESK_SET_USER, ({ user }) => {
    if (window.$woodesk) {
      window.$woodesk.setUser(user.email, {
        avatar_url: user.avatar_url,
        email: user.email,
        identifier_hash: user.hmac_identifier,
        name: user.name,
      });
      window.$woodesk.setCustomAttributes({
