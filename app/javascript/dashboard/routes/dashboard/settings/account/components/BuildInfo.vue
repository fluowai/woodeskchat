...
const latestWoodeskVersion = computed(() => {
  return currentAccount.value.latest_woodesk_version;
});

const globalConfig = useMapGetter('globalConfig/get');

const hasAnUpdateAvailable = computed(() => {
  if (!semver.valid(latestWoodeskVersion.value)) {
    return false;
  }

  return semver.lt(globalConfig.value.appVersion, latestWoodeskVersion.value);
});
...
<template>
  <div class="p-4 text-sm text-center">
    <div v-if="hasAnUpdateAvailable && globalConfig.displayManifest">
      {{
        t('GENERAL_SETTINGS.UPDATE_WOODESK', {
          latestWoodeskVersion: latestWoodeskVersion,
        })
      }}
    </div>
