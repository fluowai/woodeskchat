...
export const setBubbleText = bubbleText => {
  if (isExpandedView(window.$woodesk.type)) {
...
export const createBubbleIcon = ({ className, path, target }) => {
  let bubbleClassName = `${className} woot-elements--${window.$woodesk.position}`;
...
const handleBubbleToggle = newIsOpen => {
  ...
  if (newIsOpen) {
    dispatchWindowEvent({ eventName: WOODESK_OPENED });
  } else {
    dispatchWindowEvent({ eventName: WOODESK_CLOSED });
    chatBubble.focus();
  }
};
...
export const onBubbleClick = (props = {}) => {
  ...
  const { isOpen } = window.$woodesk;
  if (isOpen === toggleValue) return;

  const newIsOpen = toggleValue === undefined ? !isOpen : toggleValue;
  window.$woodesk.isOpen = newIsOpen;
