...
export function useBranding() {
  ...
  /**
   * Replaces "Woodesk" (any casing) in text with the installation name from
   * global config
   * @param {string} text - The text to process
   * @returns {string} - Text with "Woodesk" replaced by installation name
   */
  const replaceInstallationName = text => {
    ...
    return text.replace(/woodesk/gi, installationName);
  };
