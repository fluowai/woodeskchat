class UserSessionTrackingService
  ...
  LEGACY_MOBILE_UAS = [
    ...
    { match: %r{\AWoodesk/.*CFNetwork.*Darwin}, platform: 'iPhone', device: 'iPhone' }
  ].freeze
  ...
  def mobile_client_headers
    name = @request.headers['X-Woodesk-Client-Name']
    return nil if name.blank?

    platform = @request.headers['X-Woodesk-Platform']
    model = @request.headers['X-Woodesk-Device-Model']

    {
      browser_name: name,
      browser_version: @request.headers['X-Woodesk-Client-Version'],
      device_name: device_name_for_icon(platform, model),
      platform_name: model,
      platform_version: @request.headers['X-Woodesk-Platform-Version']
    }
  end
