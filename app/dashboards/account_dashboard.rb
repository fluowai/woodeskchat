...
class AccountDashboard < Administrate::BaseDashboard
  ...
  enterprise_attribute_types = if WoodeskApp.enterprise?
                                 attributes = {
                                   limits: AccountLimitsField
                                 }

                                 # Only show manually managed features in Woodesk Cloud deployment
                                 attributes[:manually_managed_features] = ManuallyManagedFeaturesField if WoodeskApp.woodesk_cloud?
  ...
  enterprise_show_page_attributes = if WoodeskApp.enterprise?
                                      attrs = %i[custom_attributes limits]
                                      attrs << :manually_managed_features if WoodeskApp.woodesk_cloud?
                                      attrs << :all_features
  ...
  enterprise_form_attributes = if WoodeskApp.enterprise?
                                 attrs = %i[limits]
                                 attrs << :manually_managed_features if WoodeskApp.woodesk_cloud?
                                 attrs << :all_features
                                 attrs << :captain_models
  ...
  def permitted_attributes(action)
    ...
    # Add manually_managed_features to permitted attributes only for Woodesk Cloud
    attrs << { manually_managed_features: [] } if WoodeskApp.woodesk_cloud?
