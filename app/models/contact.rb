...
class Contact < ApplicationRecord
  ...
  validate :validate_cpf_and_cnpj
  ...
  def validate_cpf_and_cnpj
    return unless custom_attributes.present?

    cpf = custom_attributes['cpf'] || custom_attributes['CPF']
    if cpf.present? && !valid_cpf?(cpf)
      errors.add(:custom_attributes, 'CPF inválido')
    end

    cnpj = custom_attributes['cnpj'] || custom_attributes['CNPJ']
    if cnpj.present? && !valid_cnpj?(cnpj)
      errors.add(:custom_attributes, 'CNPJ inválido')
    end
  end
  ...
  def valid_cpf?(cpf)
    ...
    sum1 = 0
    (0..8).each { |i| sum1 += cpf[i].to_i * (10 - i) }
    digit1 = 11 - (sum1 % 11)
    digit1 = 0 if digit1 >= 10

    sum2 = 0
    (0..9).each { |i| sum2 += cpf[i].to_i * (11 - i) }
    digit2 = 11 - (sum2 % 11)
    digit2 = 0 if digit2 >= 10
  ...
  def valid_cnpj?(cnpj)
    ...
    sum1 = 0
    (0..11).each { |i| sum1 += cnpj[i].to_i * weights1[i] }
    digit1 = 11 - (sum1 % 11)
    digit1 = 0 if digit1 >= 10

    weights2 = [6, 5, 4, 3, 2, 9, 8, 7, 6, 5, 4, 3, 2]
    sum2 = 0
    (0..12).each { |i| sum2 += cnpj[i].to_i * weights2[i] }
    digit2 = 11 - (sum2 % 11)
    digit2 = 0 if digit2 >= 10
