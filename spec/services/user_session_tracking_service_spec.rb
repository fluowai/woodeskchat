...
RSpec.describe UserSessionTrackingService do
  ...
  describe '#create_or_update!' do
    ...
    context 'with a Woodesk Mobile legacy User-Agent' do
      context 'when the UA is okhttp (Android Woodesk Mobile)' do
        let(:ua) { 'okhttp/4.9.2' }

        it 'labels the session as Woodesk Mobile on Android', :aggregate_failures do
          service.create_or_update!

          session = user.user_sessions.last
          expect(session.browser_name).to eq('Woodesk Mobile')
      ...
      context 'when the UA is CFNetwork (iOS Woodesk Mobile)' do
        let(:ua) { 'Woodesk/3759 CFNetwork/3886.100.1 Darwin/27.0.0' }

        it 'labels the session as Woodesk Mobile on iPhone', :aggregate_failures do
          service.create_or_update!
    ...
    context 'with X-Woodesk-* structured headers' do
      let(:ua) { 'Woodesk/3759 CFNetwork/3886.100.1 Darwin/27.0.0' }

      context 'when platform is ios and model is an iPhone' do
        let(:headers) do
          {
            'X-Woodesk-Client-Name' => 'Woodesk Mobile',
            'X-Woodesk-Client-Version' => '4.7.0',
            'X-Woodesk-Platform' => 'ios',
            'X-Woodesk-Platform-Version' => '18.2',
            'X-Woodesk-Device-Model' => 'iPhone 15 Pro'
          }
        end
        ...
        let(:headers) do
          {
            ...
            'X-Woodesk-Platform' => 'ios',
            'X-Woodesk-Platform-Version' => '18.2',
            'X-Woodesk-Device-Model' => 'iPad Pro 11-inch'
          }
        end
        ...
        let(:headers) do
          {
            'X-Woodesk-Client-Name' => 'Woodesk Mobile',
            'X-Woodesk-Client-Version' => '4.7.0',
            'X-Woodesk-Platform' => 'android',
            'X-Woodesk-Platform-Version' => '14',
            'X-Woodesk-Device-Model' => 'Pixel 7 Pro'
          }
        end
      ...
      context 'when X-Woodesk-Client-Name is blank' do
        let(:ua) { 'okhttp/4.9.2' }
        let(:headers) do
          {
            'X-Woodesk-Client-Name' => '',
            'X-Woodesk-Platform' => 'android',
            'X-Woodesk-Device-Model' => 'Pixel 7 Pro'
          }
        end
      ...
      context 'when no X-Woodesk-* headers are sent (real browser)' do
        let(:ua) { 'Mozilla/5.0 (X11; Linux x86_64; rv:124.0) Gecko/20100101 Firefox/124.0' }
        let(:headers) { {} }
